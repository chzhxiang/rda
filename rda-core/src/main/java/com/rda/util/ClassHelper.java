...
public class ClassHelper {
	 ...
	 * available; the ClassLoader that loaded the ClassUtils class will be used as fallback. <p>
	 * Call this method if you intend to use the thread context ClassLoader in a scenario where you
	 * absolutely need a non-null ClassLoader reference: for example, for class path resource
	 * loading (but not necessarily for <code>Class.forName</code>, which accepts a
	 ...
	 * naming rules for primitive classes. <p> Also supports the JVM's internal class names for
	 * primitive arrays. Does <i>not</i> support the "[]" suffix notation for primitive arrays; this
	 * is only supported by {@link #forName}.
	 * 
	 * @param name the name of the potentially primitive class
	 * @return the primitive class, or <code>null</code> if the name does not denote a primitive
	 *         class or primitive array class
	 */
