...
public final class ClassGenerator {
	...
	private Map<String, Constructor<?>> mCopyConstructors; // <constructor desc,constructor
															// instance>
			...
			if (mConstructors != null) {
				for (String code : mConstructors) {
					if (code.charAt(0) == ':') {
						...
						String[] sn = mCtc.getSimpleName().split("\\$+"); // inner class name
																			// include $.
	...
	private CtMethod getCtMethod(Method m) throws NotFoundException {
		return getCtClass(m.getDeclaringClass()).getMethod(m.getName(), ReflectionUtils.getDescWithoutMethodName(m));
	}
