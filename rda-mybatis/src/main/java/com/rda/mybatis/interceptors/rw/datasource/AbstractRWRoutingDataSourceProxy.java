...
public abstract class AbstractRWRoutingDataSourceProxy extends AbstractDataSource implements InitializingBean {
	...
	private class RWConnectionInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			try {
				//到此未取到target说明此语句不是通过sqlmap执行则从write db中取connection
				if(target==null){
					target=getTargetConnection(method,new Object[]{AbstractRWRoutingDataSourceProxy.WRITE});
				}
		...
		private Connection getTargetConnection(Method operation, Object[] args) throws SQLException {
			...
			if (this.target == null) {
				...
				this.target = (this.username != null) ? determineTargetDataSource(key).getConnection(this.username,
						this.password) : determineTargetDataSource(key).getConnection();
