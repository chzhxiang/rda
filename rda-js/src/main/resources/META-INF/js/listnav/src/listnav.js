define(function(require) { return function($) {
	require('cookie')($);
	var py = require('./jquery.charfirst.pinyin.js');
/*
*
		...
		var opts = $.extend({}, $.fn.listnav.defaults, options); var letters = ['_', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '-']; var firstClick = false; opts.prefixes = $.map(opts.prefixes, function(n) { return n.toLowerCase(); }); return this.each(function() {
			...
			function addClasses() {
				var str, firstChar, firstWord, spl, $this, hasPrefixes = (opts.prefixes.length > 0); $($list).children().each(function() {
					...
					if (hasPrefixes) { spl = str.split(' '); if ((spl.length > 1) && ($.inArray(spl[0], opts.prefixes) > -1)) { firstChar = spl[1].charAt(0); firstChar = py.makePy(firstChar)[0].toLowerCase(); addLetterClass(firstChar, $this, true); } }
					firstChar = str.charAt(0); firstChar = py.makePy(firstChar)[0].toLowerCase(); addLetterClass(firstChar, $this);
					} 
				});
			}
